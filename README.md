...
1. 右键桌宠 → **DAG 编辑器**，应用会在本地启动一个仅回环监听的 HTTP 服务并自动打开浏览器（地址带一次性 token）
2. 左侧节点库点击/拖拽添加节点；画布右键可打开节点/连线菜单；左键只用于选中、框选、拖拽和连线。选中节点后在右侧属性面板按 schema 编辑 `config`
3. **保存** 会做服务端校验（错误则拒绝保存并列出问题）、原子写盘 + `.bak` 备份，并立即触发热重载——运行中的 Agent 用旧图跑完本轮后自动切换新图，无需重启程序
4. 配置文件被外部修改时会弹出横幅，可选择重新加载磁盘内容或用编辑器内容覆盖
5. 服务只监听 `127.0.0.1`、随机端口 + 随机 token 鉴权，不会暴露到局域网
