...
## 2026-09-07 上下文与向量检索性能优化

### 1. DAG 上下文热路径优化

修改文件：`src/agent/agent_context.cpp`。

- `GetValue` 使用 `constFind` 单次哈希查找，替代 `contains` 与 `value` 的重复查找。
- `Overlay` 直接遍历内部键值，避免生成有序键列表、重复规范化键名与查找；自覆盖直接返回，保持快照写时复制隔离。
- `BuildDelta` 常见非别名路径直接遍历并查找差异，避免两次全量键排序；输出对象与当前或基准对象相同时保留原有先清空再计算的行为。
- 保持 `GetKeys` 的有序输出和公开接口不变。

### 2. 向量检索缓冲复用与 Top-K 部分排序

修改文件：`src/memory/vector_store.cpp`。

- 将向量读取缓冲移到查询循环外复用，避免每条记录重复分配；保留 `memcpy` 和非有限值检测，不引入未对齐浮点指针读取。
- 当 `0 < maxResults < 结果数` 时使用 `std::partial_sort`，排序阶段由全量排序的 O(N log N) 降为 O(N log K)；仍需扫描所有候选向量并保存候选结果。
- 保持分数降序、ID 升序，以及非正 K 或超量 K 返回完整有序结果的行为。

### 3. 回归测试与性能验证

修改文件：`tests/agent_dag_graph_test.cpp`、`tests/memory_embedding_test.cpp`、`CMakeLists.txt`。

- 补充上下文空值、键规范化、有序键、合并覆盖、自覆盖、增改删差分、输出别名和快照隔离测试；将上下文源码加入现有 `agent_dag_graph_tests` 目标。
- 补充向量分数并列、非正 K、超量 K、损坏 blob 与 NaN/Infinity 过滤测试，覆盖完整排序与部分排序路径。
- 重新构建后全量 CTest **22/22 通过**；主 agent 独立复跑同样全部通过（14.38 秒）。`git diff --check` 无格式错误。

独立基准使用优化前 HEAD 源码和优化后源码，以相同 MinGW-w64 GCC 13.1.0、Qt 6.9.2、`-O2 -std=c++17` 配置编译。上下文基准使用 200 个键、10,000 轮操作（查值共 2,000,000 次，合并覆盖 100 个键）；向量基准使用 5,000 条 384 维向量、每种 K 检索 50 次。最后一轮平均单次耗时如下：

| 操作 | 优化前 | 优化后 | 结果 |
| --- | --- | --- | --- |
| `GetValue` | 177.72 ns | 167.28 ns | 耗时减少约 5.9% |
| `Overlay` | 34.46 μs | 14.53 μs | 约 2.37 倍速度 |
| `BuildDelta` | 54.11 μs | 17.55 μs | 约 3.08 倍速度 |
| `QueryTopK`，K=5 | 13.22 ms | 13.25 ms | 基本持平 |
| `QueryTopK`，K=50 | 13.40 ms | 12.93 ms | 耗时减少约 3.5% |

以上为本机局部操作基准，不代表端到端对话加速比例；向量检索结果存在运行波动，Top-5 未观察到稳定收益。未调整截图线程或模型推理流程。基准源码及脚本保留于本机系统临时目录 `vpet_perf_benchmark`，不纳入仓库提交。

## 2026-09-05 工具调度层（tool.loop 微编排与纪律层，阶段 1–4）

### 1. 宏微编排分层：在 DAG 骨架内引入 LLM 动态工具调用与 ToolScheduler 纪律层

按 `docs/tool_loop_implementation_plan.md` 完成了轻量工具微编排架构开发与自动化验证。系统保持“宏编排（DAG）人定边界、微编排（tool.loop）模型选路径、纪律层（ToolScheduler）管安全”的分层原则：

新增文件：
- `include/vpet/agent/tools/itool.h` / `src/agent/tools/itool.cpp`（`ITool` 抽象接口、`_tagToolSpec`、`_tagToolCall`、`_tagToolExecutionResult`、Schema 子集校验）
- `include/vpet/agent/tools/tool_registry.h` / `src/agent/tools/tool_registry.cpp`（`ToolRegistry`，支持按 name 查找、重名拦截、按工具子集构建 OpenAI tools 声明）
- `include/vpet/agent/tools/tool_scheduler.h` / `src/agent/tools/tool_scheduler.cpp`（纪律层：权限门控、预算检查、超时 watchdog、取消传播、单飞与 SHA-256 参数哈希审计）
- `include/vpet/agent/tools/tool_loop_executor.h` / `src/agent/tools/tool_loop_executor.cpp`（`tool.loop` 节点内核状态机，管理每轮消息组装、LLM tool_calls 派发、收束作答与 plain_chat 降级）
- `include/vpet/agent/tools/native_tools.h` / `src/agent/tools/native_tools.cpp`（三大原生工具包装：`web.search`、`memory.search`、`screen.describe`）
- `include/vpet/agent/tool_loop_node.h` / `src/agent/tool_loop_node.cpp`（`tool.loop` 节点协议适配器：配置解析校验与协议键落地）
- `tests/tool_scheduler_tests.cpp`（调度器单测：权限、预算、超时、取消、单飞、审计等）
- `tests/llm_tool_calling_tests.cpp`（LLM tools stub 单测：tools 声明序列化、tool_calls 解析、4xx 降级等）
- `tests/tool_loop_tests.cpp`（循环多轮端到端 stub 测试：多轮收束、预算耗尽、terminate 提示、降级等）

修改文件：
- `include/vpet/llm/llm_client.h` / `src/llm/llm_client.cpp`：支持非流式 function calling（`_tagLlmToolCall`、OpenAI tools 请求体格式化、响应 `tool_calls` 解析、不支持 tools 降级信号）。
- `include/vpet/agent/agent_context_keys.h`：新增 `semantic.tool.calls`、`semantic.tool.trace`、`tool.loop` 节点类型。
- `include/vpet/agent/agent_runtime.h`、`src/agent/agent_runtime*.cpp`：接入 `tool.loop` 节点处理器，使用独立 `tool:<id>` 异步关联命名空间，完善析构生命周期和级联取消传播。
- `src/dageditor/agent_node_catalog.cpp`：注册 `tool.loop` 节点 schema 目录（属性面板、参数范围、权限枚举等）。
- `src/dageditor/agent_dag_validator.cpp`：增加对 `tool.loop` 节点未知内置工具名的诊断校验。
- `agent_dag_structure.example.json`：新增采用 `tool_loop` 共享节点的示例变体。
- `AGENT_CONTEXT_KEY_PROTOCOL.md`、`Structure.md`、`README.md`、`docs/tool_loop_implementation_plan.md`：同步架构图、协议表与当前实施状态。
- `CMakeLists.txt`：注册新组件和测试目标，适配现有 5 个包含 runtime 的集成测试。

实现要点与架构决策：
- **微编排安全性与收敛保证**：`ToolScheduler` 强制限制 `max_rounds`、`max_tool_calls` 及整体墙钟时间 `time_budget_ms`；预算耗尽时通过一次无 tools 的收束调用生成回复，端点违规返回 tool_calls 则直接失败中断，避免无限循环。
- **错误即结果与权限分级**：未知工具、参数不匹配或副作用工具被权限拦截时，不抛异常中断图，而是合成结构化错误回喂给 LLM，使模型具备自主解释失败并调整路径的能力。
- **并发与晚到防护**：单工具顺序单飞执行（Sequential Single-flight），分配单调递增 `executionId`，彻底滤除超时或取消后的迟到信号。
- **无感向下兼容**：默认 DAG（`agent_dag_structure.json`）保持原样不变，`conversation.history` 格式保持不变，下游 `emotion.rewrite` / `output.format` / TTS 链路对内部工具转写零感知。

测试效果：
- CTest 全部 22 个测试套件（含原有 19 项及新增 3 项）**100% 通过（22/22 passed）**。
- `git diff --check` 无格式异常。
- 真实 API、实际屏幕抓取及在线检索手工端到端验收留待实际硬件与网络环境执行。

## 2026-08-14 DAG Web 编辑器（M0–M4）

### 1. 可视化 DAG 编辑器：本地 HTTP 服务 + 浏览器前端 + 热重载闭环

本次开发按 `docs/dag_web_editor_plan.md` 实现了 ComfyUI 风格的图形化 DAG 编辑器：桌宠右键菜单「DAG 编辑器」启动仅回环监听的 HTTP 服务并自动打开浏览器，编辑保存后立即触发热重载，无需重启程序。

新增文件：

- `include/vpet/dageditor/agent_node_catalog.h` / `src/dageditor/agent_node_catalog.cpp`（M0 节点 schema 注册表，10 类节点）
- `include/vpet/dageditor/dag_document.h` / `src/dageditor/dag_document.cpp`（格式 v1.1 文档模型，未知字段无损往返）
- `include/vpet/dageditor/agent_dag_validator.h` / `src/dageditor/agent_dag_validator.cpp`（错误/警告诊断器，环检测、必填与取值范围校验）
- `include/vpet/dageditor/dag_http_request.h` / `src/dageditor/dag_http_server.cpp`（QTcpServer 最小 HTTP/1.1 + SSE 传输层）
- `include/vpet/dageditor/dag_document_store.h` / `src/dageditor/dag_document_store.cpp`（QSaveFile 原子保存、.bak 备份、revision 乐观锁、外部修改采纳）
- `include/vpet/dageditor/dag_editor_server.h` / `src/dageditor/dag_editor_server.cpp`（REST API 分发、token/Host 校验、SSE 广播、热重载触发）
- `resources/dag_editor/`（零构建前端：index.html/app.js/api.js/style.css/graph_adapter.js/widgets.js/panels/*，内嵌 litegraph.js 0.7.18 MIT）+ `dag_editor_web.qrc`
- `tests/dag_hot_reload_test.cpp`、`tests/dag_document_store_test.cpp`、`tests/dag_http_server_test.cpp`
- `scripts/smoke_dag_editor.ps1`

修改文件：

- `include/vpet/agent/agent_graph_executor.h` / `src/agent/agent_graph_executor.cpp`：快照化 `m_graphSnapshot`；`ReplaceGraphIfIdle` 返回 Applied/DeferredBusy/RejectedInvalid。
- `include/vpet/agent/agent_runtime.h` / `src/agent/runtime/*.cpp`：`RequestDagReload`、QFileSystemWatcher 监视配置文件（250 ms 防抖 + SHA-256 指纹去重）、完成点应用挂起快照。
- `CMakeLists.txt`：新增 dageditor 源文件、qrc 资源与三个测试目标。
- `src/main_window.{h,cpp}`：「DAG 编辑器」菜单项与懒创建的 `DagEditorServer`。
- `Structure.md`、`README.md`、`THIRD_PARTY_NOTICES.md`（litegraph.js MIT 声明）。

实现要点：

- **热重载语义**：进行中的一轮永远用旧图跑完，新一轮用新图；忙碌时延迟到同步完成/异步恢复/超时等完成点原子替换，失败保留旧图并发信号。
- **安全边界**：回环绑定、内核随机端口、随机 token（`X-DAG-Token` 或 `?token=`）、Host 白名单防 DNS rebinding、无 CORS 头、静态资源豁免鉴权、请求体上限 1 MB。
- **保存流程**：校验失败 400 并列出 issues（含节点高亮联动）；revision 冲突 409 携带最新文档供选择覆盖或让步；成功后自动触发热重载并在 SSE 推送结果。
- **前端**：litegraph 画布 + 左侧分类节点库（点击/拖拽添加）+ 右侧按 schema 渲染的属性面板（int/float/bool/enum/multiline/string_list 控件）+ 底部问题栏（点击定位节点）；撤销/重做、删除确认（列出受影响连线数）、PNG 导出、Ctrl+S/Z/Y。

测试效果：

- 新增 `dag_hot_reload_tests`（5 项）、`dag_document_store_tests`（8 项）、`dag_http_server_tests`（11 项，含 QNetworkAccessManager 全链路、SSE 首事件与广播帧、409 冲突、413 上限、401 鉴权）。
- ctest 18 个目标中 17 个通过；唯一失败的 `memory_embedding_tests::MeanPoolTest::LengthMismatchRejected` 为本次改动之前已存在的问题（`EmbeddingClient::MeanPoolAndNormalize`），与本模块无关。
